...
def video_hpsv3_remote(device):
    ...
    batch_size = 64
    url = "http://172.20.66.144:8080"
    # url = "http://" # xvideo的 最好别用这个
    ...
    def _fn(videos, prompts, metadata):
        ...
        for video_batch, prompt_batch in zip(videos_batched, prompts_batched):
            ...
            for video, prompt in zip(video_batch,prompt_batch):
                jpeg_images = []
                for image in video:
                ...
                data = {
                    "images": jpeg_images,
                    "prompts": [prompt]*len(jpeg_images)
                }
