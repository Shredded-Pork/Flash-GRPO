...
def sde_step_with_logprob(
    ...
    if prev_sample is None:
        if not determistic:
            ...
            #     [sample.shape[0] * 2] + list(model_output.shape[1:]),
            #     generator=generator,
...
def wan_pipeline_with_logprob(
    ...
    if determistic == False:
        import random
        sampled_indices = index
    else:
        sampled_indices = 1000000 # never match
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if i == sampled_indices - 1:
                # all_latents.append(latents.repeat_interleave(2,dim=0))
                all_latents.append(latents)
            if i == sampled_indices:
                all_latents.append(latents)
    ...
    if not return_dict:
        return (video, all_latents, all_log_probs, all_kl, sampled_indices)

    return WanPipelineOutput(frames=video), all_latents, all_log_probs, all_kl, sampled_indices